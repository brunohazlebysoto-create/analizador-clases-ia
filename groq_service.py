...
class GroqService:
    ...
    def extract_audio(self, video_path, audio_output_path="extracted_audio.mp3"):
        """Extrae el audio del video en MP3 de bajo bitrate usando ffmpeg"""
    ...
    def transcribe_audio(self, audio_path, progress_callback=None, language="es"):
        """
        Transcribe el audio con Groq Whisper Large V3.
        Retorna una lista de segmentos con timestamps.
        language: código ISO 639-1 (ej. "es", "en", "pt")
        """
        if not self.is_configured():
            raise ValueError("API Key de Groq no configurada.")

        print("Iniciando transcripción con Groq Whisper Large V3...")
        ...
        if file_size_mb > 25:
            raise ValueError(
                f"El archivo de audio es demasiado grande para la API de Groq "
                f"({file_size_mb:.2f} MB). Límite es 25MB."
            )
