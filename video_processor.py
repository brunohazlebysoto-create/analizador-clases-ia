...
def is_black_image(image_path, brightness_threshold=15.0, std_threshold=10.0):
    ...
    # Pantalla negra o de brillo muy bajo
    if mean_val < brightness_threshold:
        return True
    # Pantalla de un solo color sólido (muy poca variación de color/texto)
...
def are_images_similar(img1, img2, similarity_threshold=12.0, block_match_ratio=0.85):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            block_diff = np.mean(cv2.absdiff(block1, block2))
            if block_diff < similarity_threshold:
                matching_blocks += 1
                
    ratio = matching_blocks / total_blocks
    return ratio >= block_match_ratio
...
class VideoProcessor:
    ...
    def extract_slides(self, threshold=10.0, min_slide_duration=5.0, sample_interval=1.0, progress_callback=None):
        ...
        - min_slide_duration: Duración mínima en segundos que debe durar una diapositiva para considerarla válida.
        ...
        while t < duration:
            ...
            # Posicionar el video en el segundo 't'
            cap.set(cv2.CAP_PROP_POS_MSEC, int(t * 1000))
            ret, frame = cap.read()
            if not ret:
                break
                
            # Redimensionar y convertir a escala de grises para comparación rápida
            ...
            if last_saved_frame_gray is None:
                ...
                slides.append({
                    ...
                    "image_name": img_name,
                    "start_time": t,
                    "end_time": duration # Temporalmente hasta que encontremos la siguiente
                })
            else:
                # Calcular la diferencia absoluta con la última diapositiva guardada
        ...
        # Ajustar la duración de la última diapositiva
        if slides:
            slides[-1]["end_time"] = duration
            
        # PASO 1 POST-PROCESAMIENTO: Eliminar diapositivas negras y redistribuir su tiempo al contexto
        non_black_slides = []
        if slides:
        ...
        # PASO 2 POST-PROCESAMIENTO: Fusionar diapositivas adyacentes similares (grilla 8x8)
        merged_slides = []
        if slides:
        ...
        if slides:
            ...
            # PASO 4 POST-PROCESAMIENTO: Re-indexar IDs y renombrar archivos físicamente de forma secuencial
            for idx, slide in enumerate(slides):
                new_id = idx + 1
